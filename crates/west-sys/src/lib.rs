...
fn instantiate(config: Config) -> anyhow::Result<Instance> {
    ...
    let subscriber = tracing_subscriber::fmt::Builder::default()
        .without_time()
...
#[instrument(level = "trace", skip(store, ty, src), ret(level = "trace"))]
fn lower(
    ...
    match (src, ty) {
        ...
        (Val::String(val), Type::String) => {
            ...
            if val.is_empty() {
                ...
                unsafe { copy_nonoverlapping(val.as_ptr(), data, size) }
                unsafe { dst.write((data, size)) }
            }
            Ok(dst.as_ptr().wrapping_add(1).cast())
        }
